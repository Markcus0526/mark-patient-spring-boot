...
```
The response will contain a JWT token. This token must be included in the `Authorization` header as a Bearer token for all subsequent protected requests (e.g., `Authorization: Bearer <your_token>`).
